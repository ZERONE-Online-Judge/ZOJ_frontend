...
export default function ScoreboardReleaseControl({
  ...
  return (
    <section className="grid gap-4 rounded-lg border border-indigo-200 bg-white p-4 text-slate-800">
      {confirmationDialog}
      ...
      <p className="text-xs leading-5 text-slate-500">
        이 화면에서 공개할 순위를 조작하세요. 공개 시작 시 이 유형의 성적을
        고정하며, 변경 사항은 참가자 화면과 프레젠테이션에 자동으로 반영됩니다.
        프레젠테이션은 조작 없이 결과만 표시합니다.
      </p>
        ...
        {release?.mode !== 'all' ? (
          <button
            ...
            onClick={async () => {
              if (
                await confirmAction(
                  `${divisionName}의 남은 순위를 모두 공개할까요?`,
                  {
                    title: '순위 공개 확인',
                    confirmLabel: '공개',
                    tone: 'primary',
